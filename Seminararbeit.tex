...
The paper \cite{Jacobs2018} introduces an intent-refinement process that uses machine learning and feedback from the operator to translate intents into network configurations. The key insight of this refinement process is an intermediate intent representation that is close to natural language and suitable to collect feedback from the user but is structured enough to enable accurate translations. The presented prototype interacts with a network operator using natural language and translates the input to the intermediate representation before translating to SDN commands.
